...
main()
{
   ...
   flow[0] = 0; flow[1] = 0; purge[0]= 0; purge[1] = 0;
      ...
      auto int retval1;
		...
		switch(*token) // Read Command
		{
         ...
         case 72:   //Control Primary Ring Parameters
            ...
            if(my_port == 1001) {retval1 = sock_fastwrite(&sock[1],message,strlen(message)); }

            if(my_port == 1002) {retval1 = sock_fastwrite(&sock[2],message,strlen(message));}
