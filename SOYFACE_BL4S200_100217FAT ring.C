...
#use "BLxS2xx.lib"// tam0509 Edits 05082009 Changed Ozone settings to target instead of tracking and hours of 1000 - 1800
// tam0509 Modified from SOYFACE050307.c
...
//#define WIRED
# endif /* NETWORK */ # define EXTDA

# define STDIO_DEBUG_SERIAL SADR# define STDIO_DEBUG_BAUD 57600# define STDIO_DEBUG_ADDCR# define Polled 0 // Set 0 to dump data per second/minute, 1 to wait for command
...
# define STARTTIME_OZONE 9# define STOPTIME_OZONE 17# define STARTTIME_CO2 8# define STOPTIME_CO2 17
...
# ifdef NETWORK# define TCPCONFIG 1# define _PRIMARY_STATIC_IP "192.168.1."#
...
define IFC_WIFI_ENCRYPTION IFPARAM_WIFI_ENCR_WEP# define IFC_WIFI_WEP_KEYNUM 0# define IFC_WIFI_WEP_KEY0_HEXSTR "101a08f9786683025d96eb04d3"

#
endif /* WIRELESS */

# define sethostid# define UDP_SOCKETS 6 // allow enough for downloader and DHCP
# define MAX_UDP_SOCKET_BUFFERS 6# define LOCAL_PORT 425# define REMOTE_IP "255.255.255.255" /*broadcast*/ #
...
# define MAX_TCP_SOCKET_BUFFERS 6# define ETH_MAXBUFS 12

# use "dcrtcp.lib"#
ifdef IPDOWNLOAD# define UDPDL_LOADER "Z:/Z-World/UDPDL/Loaders/pdl-generic-D.bin"#
use "udpdownl.lib"#
endif /* IPDOWNLOAD */
...
# memmap xmem
int i;#
ifdef FAT# define FAT_USE_FORWARDSLASH# define FAT_BLOCK# use "fat16.lib"#
use "dcrtcp.lib"
...
#
define EINBUFSIZE 127# define EOUTBUFSIZE 127# define FINBUFSIZE 127# define FOUTBUFSIZE 127# define MAX_SENTENCE 100

# define SETRAW 16 //offset to get raw analog data
# define INS 1# define OUTS 0# define BANK_A INS //bank A inputs
...
main() {
  ...
  settings[10] = 237; //Unit ID IP Address 200
  ...
  for (i = 0; i < N_CHANNELS + 1; i++) {
    ...
    if (LAYER[i] == 2) {
      ...
      AD_OFFSET[i] = 0;
      DA_MULT[i] = 0;
    } else if (LAYER[i] == 1) { //Layertype 1 - Ozone Settings
      ...
      AD_OFFSET[i] = 0;
      DA_MULT[i] = .5;
    } else if (LAYER[i] == 0) { //Layertype 0 - CO2 Settings
      FAINT[i] = -0.000015; //
      FAPRO[i] = -0.0008; //
      FADIF[i] = -0.016; //
      FCW[i] = 0.15; //
...
void sector_select(void) {
  ...
  if (purge) {
    if (purge_off <= (int) SEC_TIMER) {
      digOut(1, TURNOFF); // DIGOUT_H in Tim's version
      digOut(2, TURNOFF); // DIGOUT_H in Tim's version
      purge = 0;
    }
  }
  ...
  for (chn = 0; chn < N_CHANNELS; chn++) {
    ...
    if (LAYER[chn] == 1) // OZONE CONTROL LAYER
    {
      ...
      if ((ora >= STARTTIME_OZONE && ora < STOPTIME_OZONE) && ozonator_rem) {
        digOut(0, TURNON); //CHANGE2003	// DIGOUT_H in Tim's version
        ozone_power = 1;
      } else {
        digOut(0, TURNOFF); // DIGOUT_H in Tim's version
        ozone_power = 0;
      }
      ...
      if ((ora >= STARTTIME_OZONE && ora < STOPTIME_OZONE) && ozonator_loc && ozonator_rem && !purge && !delay_restart) // Esegue le operazioni solo di giorno 18.08.2000
      {
        if (nSel >= MOBILE && s45 > 0.5) {
          ...
          digOut(1, TURNON); //Turn on AC flow	// DIGOUT_H in Tim's version
          MobSum = -1000;
          for (wI = 1; wI <= 8; wI++) {
            //printf("%f\n",MobSum);
            if (MobDir[wI] > MobSum) {
          ...
          //nSel=0;
          flow = 1;
          if (ozonator_rem) digOut(0, TURNON); //CHANGE2003	// DIGOUT_H in Tim's version
          else digOut(0, TURNOFF); // DIGOUT_H in Tim's version
        ...
        digOut(0, TURNOFF); //Turn off Ozone Generator	// DIGOUT_H in Tim's version
        if (s45 < 0.5) {
          delay_restart = TRUE;
          delay_restart_time = SEC_TIMER + delay_time;
        }
...
void readSerialE(void) {
  ...
  if (input_char == '\r') {
    ...
    readString_E();
  } else if (input_char > 100) {} else if (input_char > 0) {
...
void readString_E(void) //CONNECTION TO OP6800 DISPLAY
{
  ...
  case 79: // O,1Set Ozone
    ...
    if ((ora >= STARTTIME_OZONE && ora < STOPTIME_OZONE) && ozonator_rem) digOut(0, TURNON); // DIGOUT_H in Tim's version
    else digOut(0, TURNOFF); // DIGOUT_H in Tim's version
...
//tam0509 if ((ora>sunr && ora<suns) && ozonator_loc && ozonator_rem && !purge && !delay_restart)   // Esegue le operazioni solo di giorno 18.08.2000
if ((ora>10 && ora<18) && ozonator_loc && ozonator_rem && !purge && !delay_restart)   // Esegue le operazioni solo di giorno 18.08.2000
	{
        if (nSel>=MOBILE && s45>0.5)
        {
...
else
{
	...
	{
   	//tam0509 if (ora<sunr || ora>suns || !ozonator_loc || !ozonator_rem || !flow  )   // If nighttime or ozone off
      if (ora<10 || ora>18 || !ozonator_loc || !ozonator_rem || !flow  )   // If nighttime or ozone off
	  	{
	  	fVs[chn]=0;
      }
      else // Daytime and ozone on
...
for(chn=0;chn < N_CHANNELS+1;chn++)
   ...
   if(LAYER[chn]==1)  // O3 CONTROL LAYER
		{
		//tam0509 if (ora<sunr || ora>suns)   // Esegue le operazioni solo di giorno 18.08.2000
      if (ora<10 || ora>18)   // Esegue le operazioni solo di giorno 18.08.2000
			{
      	//anaOutVolts( DA_Channel[chn], 0);
          #ifdef EXTDA
          rn_anaOutVolts(device0, DA_Channel[chn],  0, &DacCalTable1, 0);
...
void readString_E(void) //CONNECTION TO OP6800 DISPLAY
{
	...
	case 79: // O,1Set Ozone
            ...
            ozonator_rem = atoi(token);	//set ozone
				//tam0509 if ((ora>sunr && ora<suns)  && ozonator_rem ) digOut(ChanAddr(RELAY, 0),TURNON);
            if ((ora>10 && ora<18)  && ozonator_rem ) digOut(0,TURNON);
            else digOut(0,TURNOFF);
